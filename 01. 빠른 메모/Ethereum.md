...
- PoS(proof-of-stake) 방식으로 결정된다.
	- Validator는 32ETH를 stake 해야 될 수 있다.
	- Validator는 새로운 트랜잭션은 mempool에 계속 넣어둔다.
- PoS인 만큼, Proposer는 정말 랜덤으로 설정되어야 한다.
	- RANDAO 라는 방식으로 설정된다.
	- Proposer는 다음 일들을 처리한다.
		- 블록 제안(mempool에서 꺼내서 만듬)
		- 트랜잭션 처리 (실행)
		- 전파 (전파된 트랜잭션은 다른 노드들에 의해 재실행되고 검증된다. 확인되면 찬성투표를 한다.)
...
- Jutification / Finalization
	...
	- 서로 충돌하는 두 블록은 같이 최종화될 수 없다. (안정성 증명)
		- 만약 두 블록의 높이가 같다면, Casper FFG의 높이가 같으면 안된다는 조건을 어긴 것이 된다.
		- 한 블록의 높이가 더 높다면, 그 블록이 최종화되기 위해 이전 블록들도 최종화가 되었을 텐데, 그 중 하나는 높이가 같은 것이며, 이는 Casper FFG의 높이가 같으면 안된다는 조건을 어긴 것이 된다.
	- 
- PoS를 공격해보자.
